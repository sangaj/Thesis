...
* Kalman Filter Model
    + Description 
    + Simple model in Mathematical Form
...
* $u(t)$ is vectors containing all locations and all input control features
...
* $V_t^{t^\ast}$ refers to the state covariance matrix at time $t$ given the observations vector and input variables from time one to time $t^\ast$.
...
* $V_{t+1,t}^T$ is the covariance of the joint posterior distribution 


##Kalman Smoother


<center>![](images/smoother.png){ width=60% }</center>
...
1. Give initial value A,B,Q,R,$\pi_1$,$V_1$ to start the algorithm
2. Implement the Kalman recursion given A,B,Q,R,$\pi_1$,$V_1$ and $z_{1:T}$ to calculate the required quantities $E[y(t)|z_{1:T}]$, $E[y(t)y(t-1)^{'}|z_{1:T}]$ and $E[y(t)y(t)^{'}|z_{1:T}]$.
3. Maximize the expected log likelihood $\Phi$ to find the updated new value of each row in A, each row in B,Q,R,$\pi_1$,$V_1$ and row-wise consolidation for A and B
...
* Equations for B
     ...
     + Indicator matrix in the equations  
...
<center>![](images/likelihood.png)</center>
...
* Greedy EM algorithm to respect the spatial assumption
    ...
    + Only approximate local optimization
